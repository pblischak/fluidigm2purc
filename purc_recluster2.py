...
"""
NB: This script was modified from its original version by Paul D. Blischak.
The intention of this modification is to reduce the number of rounds
of clustering and chimera detection from 5 to 3.

Original commit: 6afa1e270c090c40f5cbf2a0c03c96dda1f3b031
"""
...
Example: ./purc_recluster2.py -f purc_3_annotated.fa -o recluster -c 0.95 0.975 -s 5 5 --clean
...
def SplitBy(annotd_seqs_file, split_by = "locus-taxon", Multiplex_perBC_flag=False, Clust_particular=False,
            locus_to_clust='', specimen_to_clust=''):
	...
	Clust_particular to control for specific specimen or locu to cluster"""
...
def IterativeClusterDechimera(annotd_seqs_file, clustID, clustID2, sizeThreshold, sizeThreshold2, verbose_level = 1): # M_p_barcode was set to FALSE, whcih led to splitting by taxon instead of barcode
	...
	for locus_folder in all_folders_loci: # locus_folder = locus name
		...
		if not os.stat(locus_folder + ".fa").st_size == 0: # ie, the file is not empty
			...
			for taxon_folder in all_folders_taxon:
				...
				sys.stderr.write("Working on " + taxon_folder + ": 1st clustering")
				...
				sys.stderr.write(", 1st chimera detection")
				...
				sys.stderr.write(", 2nd clustering")
				...
				sys.stderr.write(", 2nd chimera detection")
				...
				for each_cluster in ClusterToSeq_dict2:
					if len(ClusterToSeq_dict2[each_cluster]) >= int(sizeThreshold2):
						cluster_seq_file = open(each_cluster, 'w')
						for seq in ClusterToSeq_dict2[each_cluster]:
							cluster_seq_file.write('>' + seq + '\n' + str(SeqDict[seq].seq) + '\n')
						cluster_seq_file.close()
						new_seq_name = taxon_folder + '_' + each_cluster + ';size=' + str(len(ClusterToSeq_dict2[each_cluster])) + ';'
				...
				sys.stderr.write(", final clustering")
				...
				usearch_cline = "%s -cluster_fast %s -id %f -gapopen 3I/1E -consout %s -uc %s -sizein -sizeout" % (Usearch, taxon_folder + '_Cluster_FinalconsensusSs.fa', clustID2, taxon_folder + '_Cluster_FinalconsensusSsC' + str(clustID2) + '.fa', taxon_folder + '_Cluster_FinalconsensusSsC' + str(clustID2) + '.uc')
				process = subprocess.Popen(usearch_cline, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
				(out, err) = process.communicate() #the stdout and stderr
				sys.stderr.write(", final chimera detection")
				usearch_cline = "%s -uchime_denovo %s -abskew %s -nonchimeras %s -uchimeout %s" % (Usearch, taxon_folder + '_Cluster_FinalconsensusSsC' + str(clustID2) + '.fa', abskew, taxon_folder + '_Cluster_FinalconsensusSsC' + str(clustID2) + 'dCh.fa', taxon_folder + '_Cluster_FinalconsensusSsC' + str(clustID2) + 'dCh.uchime')
				process = subprocess.Popen(usearch_cline, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
				(out, err) = process.communicate() #the stdout and stderr
				...
				## Count number of chimera seq found by parsing 'outFile_uchime' ##
				chimera_count3 = 0
				uchime_out = open(taxon_folder + '_Cluster_FinalconsensusSsC' + str(clustID2) + 'dCh.uchime', 'rU')
				for line in uchime_out:
					line = line.strip('\n')
					if line.split('\t')[-1] == 'Y':
						chimera_count3 = chimera_count3 + 1
				LocusTaxonCountDict_chimera[taxon_folder, locus_folder] = [chimera_count1, chimera_count2, chimera_count3]

				## Count clustered seq and store in LocusTaxonCountDict_clustd as {('C_dia_5316', 'ApP'): 28} for example ##
				try:
					clustered_seq_file = parse_fasta(taxon_folder + '_Cluster_FinalconsensusSsC' + str(clustID2) + 'dCh.fa')
				...
				sed_cmd = "sed 's/>/>%s_/g' %s > %s" % (taxon_folder, taxon_folder + '_Cluster_FinalconsensusSsC' + str(clustID2) + 'dCh.fa', taxon_folder + '_Cluster_FinalconsensusSsC' + str(clustID2) + 'dCh_renamed.fa')
...
required.add_argument('-c','--clustering_identities', type=float, nargs=2, metavar='\b', required=True,
               help='The similarity criterion for the first and second USEARCH clustering',
               default=[0.997, 0.995])
...
log = open(log_file, 'w')
log.write(logo + '\n')
log.write('purc_recluster.py ' + annotated_file + ' ' + masterFolder + ' ' + str(clustID) + ' ' + str(clustID2) + ' ' + str(sizeThreshold) + ' ' + str(sizeThreshold2) + '\n\n')
...
LocusTaxonCountDict_clustd, LocusTaxonCountDict_unclustd, LocusTaxonCountDict_chimera = IterativeClusterDechimera(annotated_file, clustID, clustID2, sizeThreshold, sizeThreshold2)
...
for each_locus in locus_list:
	...
	for each_taxon in set(taxon_list):
		try:
			count_output.write('\t' + each_taxon + '\t' + str(LocusTaxonCountDict_chimera[each_taxon, each_locus][0]) + '\t' + str(LocusTaxonCountDict_chimera[each_taxon, each_locus][1]) + '\t' + str(LocusTaxonCountDict_chimera[each_taxon, each_locus][2]))
		except:
			count_output.write('\t' + each_taxon)
